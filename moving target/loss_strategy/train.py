...
def dqn():
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if np.mean(scores_window) >= max_score:
            ...
            torch.save(agent.qnetwork_local.state_dict(), 'checkpoint_l_' +
                       str(agent.loss_)+"_c_"+str(agent.counter)+'.pth')
...
np.savetxt("scores_l_"+str(agent.loss_)+"_c_" +
           str(agent.counter)+".txt", scores_dqn_np)
...
train_info_dictionary = {'algorithm': 'DQN_moving_target_loss', 'eps_start': eps_start, 'eps_end': eps_end,
...
train_info_file = open('info_l_'+str(agent.loss_) +
                       "_c_"+str(agent.counter)+'.json', 'w')
